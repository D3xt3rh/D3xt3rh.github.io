# Mohammad Saalim - Portfolio Website

A modern, responsive portfolio website showcasing my professional experience, skills, projects, and achievements as a Software Engineer specializing in cloud cost optimization, microservices, and distributed systems.

🌐 **Live Demo:** [https://d3xt3rh.github.io](https://d3xt3rh.github.io)

## ✨ Features

- **Modern & Responsive Design** - Clean, professional UI with smooth animations and gradients
- **Fully Responsive** - Optimized for desktop, tablet, and mobile devices
- **Smooth Scrolling Navigation** - Easy navigation between sections
- **Professional Sections:**
  - Home/Introduction
  - About Me
  - Professional Experience
  - Technical Skills
  - Projects
  - Education
  - Certifications & Awards
  - Publications
  - Contact Form

## 🛠️ Technologies Used

- **HTML5** - Semantic markup
- **CSS3** - Modern styling with CSS variables, gradients, and animations
- **Boxicons** - Icon library for social media and UI elements
- **Google Fonts** - Poppins font family
- **Formspree** - Contact form handling

## 🚀 Getting Started

### Prerequisites

- A modern web browser (Chrome, Firefox, Safari, Edge)
- Python 3.x (for local development server) - Optional

### Local Development

1. **Clone the repository**
   ```bash
   git clone https://github.com/D3xt3rh/D3xt3rh.github.io.git
   cd D3xt3rh.github.io
   ```

2. **Open directly in browser**
   ```bash
   open index.html
   ```

3. **Or run a local server** (Recommended)
   ```bash
   # Using Python
   python3 -m http.server 8000
   
   # Using Node.js (if installed)
   npx http-server -p 8000
   ```

4. **View in browser**
   - Navigate to `http://localhost:8000`

## 📁 Project Structure

```
D3xt3rh.github.io/
│
├── index.html          # Main HTML file
├── index.css           # Stylesheet with modern design
├── README.md           # Project documentation
│
└── img/                # Image assets
    ├── Profile.jpg     # Profile picture
    └── skills.jpg      # Skills section image
```

## 🎨 Design Features

- **Color Scheme:** Modern indigo/purple gradient theme
- **Typography:** Poppins font family for clean readability
- **Layout:** Grid-based responsive layout
- **Animations:** Smooth hover effects and transitions
- **Cards:** Glassmorphism-style cards for sections

## 📝 Sections Overview

### Home
- Introduction and professional title
- Social media links
- Profile picture

### About
- Professional summary
- Key achievements and expertise

### Experience
- **CloudKeeper** (Sep 2025 - Present)
  - Cloud cost optimization
  - AWS/GCP services
  - SpotBot optimization engine
- **Samsung Research Institute** (June 2022 - Sept 2024)
  - Microservices development
  - RESTful API engineering
  - Performance optimization

### Skills
- Programming Languages: Java, Python, JavaScript, C++
- Databases: MySQL, Redis, MongoDB
- Frameworks: Spring Boot, Hibernate, Maven, Jenkins
- Cloud Platforms: AWS, GCP, Azure
- Tools & Technologies: Docker, Kubernetes, Kafka, RabbitMQ, etc.

### Projects
- **Billing and Payment System** - Distributed B2B billing platform
- **SpotBot Optimization Engine** - AWS ECS Fargate cost optimization

### Education
- B.Tech in Electronics and Communication (GPA: 9.45/10)
- Jamia Millia Islamia, New Delhi

### Certifications & Awards
- CloudKeeper Quarter Team Award (Q2 2026)
- Professional and Advanced SWC
- Samsung Incredibles Award (Q3 2023)

## 🔧 Customization

To customize this portfolio for your own use:

1. **Update Personal Information**
   - Edit `index.html` and replace personal details
   - Update social media links
   - Replace profile image in `img/Profile.jpg`

2. **Modify Colors**
   - Edit CSS variables in `index.css`:
     ```css
     :root {
       --first-color: #6366F1;  /* Primary color */
       --second-color: #0F172A;  /* Text color */
       --bg-color: #F3F4F6;      /* Background color */
     }
     ```

3. **Update Contact Form**
   - Replace Formspree endpoint in the contact form section

## 📧 Contact

- **Email:** msaalim1998@gmail.com
- **LinkedIn:** [linkedin.com/in/mohammad-saalim](https://www.linkedin.com/in/mohammad-saalim/)
- **GitHub:** [github.com/d3xt3rh](https://github.com/d3xt3rh)
- **Portfolio:** [d3xt3rh.github.io](https://d3xt3rh.github.io)

## 📄 License

This project is open source and available under the [MIT License](LICENSE).

## 🙏 Acknowledgments

- Design inspiration from modern portfolio templates
- Icons provided by [Boxicons](https://boxicons.com/)
- Fonts by [Google Fonts](https://fonts.google.com/)

---

**Built with ❤️ by Mohammad Saalim**
